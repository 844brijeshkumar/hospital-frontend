    ...
    <div className="w-full max-w-4xl mx-auto">
      <div className="relative overflow-hidden bg-white/10 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-white/20">
        <div className="mb-4 text-center">
          <h3 className="text-xl font-bold text-white mb-2">Our Trusted Healthcare Partners</h3>
          <p className="text-white/90 text-sm">Working together for your well-being</p>
        </div>
        
        <div className="relative overflow-hidden">
          <div className="flex animate-scroll-infinite space-x-6">
            {duplicatedCompanies.map((company, index) => (
              <CarouselItem key={`${company.id}-${index}`} company={company} index={index} />
            ))}
          </div>
        </div>
