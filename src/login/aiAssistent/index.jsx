...
import { SendHorizonal, Volume2, Sparkles, MessageSquarePlus, Mic, MicOff, Loader2 } from 'lucide-react';
...
const App = () => {
  ...
  const pcmToWav = (pcmData, sampleRate) => {
    ...
    // RIFF identifier
    dataView.setUint32(offset, 0x52494646, false); offset += 4;
    // file length
    dataView.setUint32(offset, 36 + pcmData.byteLength, true); offset += 4;
    // RIFF type
    dataView.setUint32(offset, 0x57415645, false); offset += 4;
    // format chunk identifier
    dataView.setUint32(offset, 0x666d7420, false); offset += 4;
    // format chunk length
    ...
    dataView.setUint32(offset, sampleRate * 2, true); offset += 4;
    // block align (channels * bytes per sample)
    dataView.setUint16(offset, 2, true); offset += 2;
    // bits per sample
    dataView.setUint16(offset, 16, true); offset += 2;
    // data chunk identifier
    dataView.setUint32(offset, 0x64617461, false); offset += 4;
    // data chunk length
  ...
  const handleApiCall = async (prompt, modelName = 'gemini-2.5-flash-preview-05-20', conversationHistory = [], responseSchema = null) => {
    let payload = {
  ...
  const sendMessage = async (messageText) => {
    ...
    const responseText = await handleApiCall(messageText, 'gemini-2.5-flash-preview-05-20', conversationHistory);
    setIsTyping(false);
    setMessages(currentMessages => [...currentMessages, { sender: 'bot', text: responseText }]);
  };
  ...
  const handleSummarize = async () => {
    ...
    const chatText = messages.slice(1).map(msg => `${msg.sender}: ${msg.text}`).join('\n');
    ...
    setIsTyping(false);
    setMessages(currentMessages => [...currentMessages, { sender: 'bot', text: `✨ **Summary:** ${summaryText}` }]);
  };
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (audioData && mimeType && mimeType.startsWith("audio/")) {
          ...
          console.error("TTS response structure is unexpected or missing audio:", result);
  ...
  const handleGenerateQuestions = async () => {
    ...
    const questions = await handleApiCall(prompt, 'gemini-2.5-flash-preview-05-20', [], schema);
  ...
  const toggleSpeechRecognition = () => {
    ...
    if (!('webkitSpeechRecognition' in window)) {
      setMessageBox({ visible: true, message: "Speech recognition is not supported by your browser. Please try Chrome." });
      return;
    }
          ...
          <div className="flex justify-start">
            <div className="max-w-xs md:max-w-md lg:max-w-lg p-4 bg-[#D8EFEA] dark:bg-gray-700 rounded-3xl rounded-bl-none">
              <div className="flex space-x-2">
                <div className="w-2 h-2 bg-[#5A9992] rounded-full animate-wave-typing" style={{ animationDelay: '0s' }}></div>
                <div className="w-2 h-2 bg-[#5A9992] rounded-full animate-wave-typing" style={{ animationDelay: '0.2s' }}></div>
                <div className="w-2 h-2 bg-[#5A9992] rounded-full animate-wave-typing" style={{ animationDelay: '0.4s' }}></div>
              </div>
            </div>
          </div>
        )}
      ...
      <form onSubmit={handleSend} className="mt-4">
        <div className="flex items-center space-x-2 bg-white dark:bg-gray-800 p-3 rounded-xl shadow-lg">
          <input
            ...
            placeholder={isRecording ? 'Listening...' : 'Type your medical query...'}
          ...
          <button
            ...
            className={`p-3 text-white rounded-xl transition-colors duration-300 disabled:bg-gray-400 ${
              isRecording ? 'bg-red-500 hover:bg-red-600' : 'bg-[#5A9992] hover:bg-[#4A817A]'
            }`}
            disabled={isTyping}
          ...
          <button
            ...
            disabled={isTyping || isGeneratingQuestions || messages.length <= 1 || messages[messages.length-1].sender !== 'bot'}
            title="Generate follow-up questions"
            aria-label="Generate follow-up questions"
          >
            {isGeneratingQuestions ? <Loader2 size={20} className="animate-spin" /> : <MessageSquarePlus size={20} />}
          </button>
          <button
            type="button"
